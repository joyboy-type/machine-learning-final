...
向测试集特征注入不同强度的高斯噪声（noise = std × level），观察模型精度变化：

| 噪声强度 | SVM | XGBoost | MLP | Random Forest |
|:--------:|:---:|:-------:|:---:|:-------------:|
| 0.00（无噪声） | 0.9342 | 0.9285 | 0.9241 | 0.9233 |
| 0.01（轻微） | 0.9342 | 0.9298 | 0.9237 | 0.9217 |
| 0.05（中等） | **0.9354** | 0.9294 | 0.9233 | 0.9269 |
| 0.10（较强） | 0.9314 | 0.9233 | 0.9197 | 0.9229 |
| **最大下降** | **-0.28%** | **-0.56%** | **-0.44%** | **-0.16%** |

**分析：**

1. **SVM 在高噪声下保持最高绝对精度**（0.9314），且在中低噪声下精度几乎无损。有趣的是 0.05 噪声下精度微升（+0.12%），这可能是噪声作为"测试时数据增强"修正了某些边界样本。
2. **MLP 对噪声最敏感**（下降 0.44 个百分点），符合神经网络对输入扰动敏感的已知特性。
3. 四类算法在 0.10 高强度噪声下精度下降均控制在 0.6% 以内，表现出了良好的鲁棒性。几何特征的分类边界本身具有一定的容错能力——轻微扰动不会改变豆粒的整体形状特征。
4. Random Forest 表现最稳定（仅下降 0.16%），但绝对精度最低。

鲁棒性曲线图见 `output/figures/robustness_curves.png`。
...
| 10 | 噪声鲁棒性曲线 | `output/figures/robustness_curves.png` |
| 11 | 训练/验证 Loss 曲线 | `output/figures/loss_curves.png` |
| 12 | 过拟合分析（训练vs测试） | `output/figures/overfitting_analysis.png` |
